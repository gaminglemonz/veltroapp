    ...
    return (
        <div className="text-white bg-slate-900 h-screen overflow-x-hidden m-0">
            ...
            <div className="bg-slate-800 m-4 p-3 rounded-xl flex justify-end">
                <a href="/create-room" className="material-icons p-3 bg-transparent rounded-full cursor-pointer select-none transition duration-500 hover:bg-slate-600 hover:bg-opacity-65">add</a>
                <i className="material-icons p-3 bg-transparent rounded-full cursor-pointer select-none transition duration-500 hover:bg-slate-600 hover:bg-opacity-65">grid_view</i>
                <i className="material-icons p-3 bg-transparent rounded-full cursor-pointer select-none transition duration-500 hover:bg-slate-600 hover:bg-opacity-65">search</i>
            </div>
            <div className="fade-in mt-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-6">
                {rooms.map((room) => {
                    const roomIcon = (typeof room.icon === 'string' && room.icon) ? room.icon.replace('/public', '..') : '/images/default-icon.png';
                    const roomBanner = (typeof room.banner === 'string' && room.banner) ? room.banner.replace('/public', '..') : '/images/default-banner.png';
                    return (
                        <div key={room.id} className="bg-slate-800 w-full rounded-lg shadow-lg overflow-hidden transform transition-all duration-300 hover:scale-102 hover:shadow-xl">
                            <div className="relative group">
                                ...
                                <img
                                    className="absolute top-2 left-2 w-10 h-10 rounded-full border-2 border-slate-800 shadow-md transition-transform group-hover:scale-105"
                                    src={roomIcon}
                                    alt="Room Icon"
                                />
                                <span
                                    className={`absolute top-2 right-2 px-2 py-0.5 rounded-full text-xs font-semibold ${
                                        room.visibility === 'public' ? 'bg-green-500' : 'bg-red-500'
                                    }`}
                                >
                                    {room.visibility}
                                </span>
